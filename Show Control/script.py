...
param_states = Parameter({'title': 'Show States', 'order': next_seq(), 'schema': {'type': 'array', 'items': {
  'type': 'object', 'title': 'State', 'properties': {
    'state': {'type': 'string', 'hint': 'Standard'}}}}})
...
def initLocalAction():
  ...
  create_local_action('Show', handler, {'title': 'Show', 'order': next_seq(), 'schema': {'type': 'string', 'enum': [show.get('state') for show in param_states]}})
  create_local_event('Show', {'title': 'Show', 'order': next_seq(), 'schema': {'type': 'string', 'enum': [show.get('state') for show in param_states]}})

def initRemoteActions():
  for show in param_states:
    create_remote_action(show.get('state'), suggestedNode=param_node, suggestedAction=show.get('state'))
